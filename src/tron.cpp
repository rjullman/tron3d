...
static const double VIDEO_FRAME_DELAY = 1./15.; // 25 FPS
static const bool FIXED_FRAME_RATE = false;

// angle of rotation for the camera direction
float angle = 0.0f;
// actual vector representing the camera's direction
float lx=1.0f,ly=0.0f;
// XY position of the camera
float x=-21.0f, y=0.0f;
...
void LoadCamera(R3Camera *camera)
{
  ...
  camera->towards.SetX(x+lx);
  camera->towards.SetY(y+ly);
...
/*void DrawParticles(R3Scene *scene)
{
  ...
  if (save_video || FIXED_FRAME_RATE) { // in video mode, the time that passes only depends on the frame rate ...
...
void releaseKey(int key, int x, int y) {
  ...
  switch (key) {
    case 'a' : deltaAngle = 0.0f;break;
    case 'd' : deltaAngle = 0.0f;break;
    case 'w' : deltaMove = 0;break;
