...
void R2Image::
Vignette(double inner_radius, double outer_radius)
{
  // Apply vignetting to image.
  // Darken the corners of the image, as observed when using 
  // lenses with very wide apertures.  The image should be perfectly clear 
  // up to inner_radius, perfectly dark (black) at outer_radius and beyond, 
  // and smoothly increase darkness in the circular ring between. 
  // Both radii are specified as multiples of half the length of the 
  // image diagonal (so 1.0 is the distance from the image center to 
  // the corner). 

  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  fprintf(stderr, "Vignette() not implemented\n");
}



void R2Image::
EqualizeHistograms(void)
{
  // Increase the contrast of an image by "equalizing" its luminance histogram.  
  // Remap the pixel intensities so that the final histogram is flat. 
  // A low contrast image usually clumps most pixels into a few tight clusters 
  // of intensities. Histogram equalization redistributes the pixel intensities 
  // uniformly over the full range of intensities [0, 1], while maintaining 
  // the relationship between light and dark areas of the image. For a color 
  // image, convert to HSV and equalize the value channel.

  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  fprintf(stderr, "EqualizeHistograms() not implemented\n");
}
...
void R2Image::
WhiteBalance(double red, double green, double blue)
{
  // Transform colors in image so that (red, green, blue) 
  // is mapped to white (1, 1, 1)

  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  fprintf(stderr, "WhiteBalance() not implemented\n");
}
...
TosRGB(void) 
{
  // Clamps the image to [0..1] and converts from linear to sRGB;
  for (size_t i = 0; i < (size_t) (width * height); i++) {
    for (int channel = 0; channel < 3; channel++) {
      // Clamp
      double& c = pixels[i][channel];
      if (c < 0) c = 0;
      if (c > 1) c = 1;
	...
	const static double a = 0.055;
	const static double gamma = 2.4;
	c = (1+a) * pow(c, 1. / gamma) - a;
      }
    }
  }
}
...
{
  // Convolve with an arbitrary filter

  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  fprintf(stderr, "Convolve() not implemented\n");
}
...
{
  // Multiply passed image with this image.

  // MAY FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE) (NO CREDIT FOR ASSIGNMENT)
  fprintf(stderr, "Multiply not implemented\n");
}
...
void R2Image::
FuseExposures(int nimages, R2Image **images, double wc, double ws, double we)
{
  // Combine this image with ones passes as arguments by taking a weighted
  // average of colors at each pixel, where the weights for each
  // pixel are determined by (the contrast raised to the power wc) times
  // (the saturation raised to the power ws) times (the well-exposedness raised
  // to the power we), as described in "Exposure Fusion" by Tom Mertens, Jan Kautz,
  // and Frank Van Reeth.

  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  fprintf(stderr, "FuseExposures not implemented\n");
}
...
{
  // Reduce the dimensions of an image by seam carving 
  // as described in: Shai Avidan and Ariel Shamir,
