...
class R2Image {
 public:
  ...
  void Vignette(double inner_radius, double outer_radius);
  void EqualizeHistograms(void);

  // Color operations
  void BlackAndWhite(void);
  void ChangeSaturation(double factor);
  void WhiteBalance(double red, double green, double blue);
  void TosRGB(void);
  ...
  void Convolve(const R2Image& filter);
  ...
  void FuseExposures(int nimages, R2Image **images, double wc, double ws, double we);
  ...
  void SeamCarve(int width, int height);
  ...
  int WriteTXT(const char *filename) const;
